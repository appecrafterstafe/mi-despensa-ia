// api/receta.js (Código de servidor corregido)
export default async function handler(req, res) {
    // 🔓 CORRECCIÓN DE CORS: Permitimos solicitudes desde cualquier visor externo
    ...
    // Manejar la verificación previa obligatoria del navegador (Preflight OPTIONS)
    ...
    try {
        ...
        if (data.candidates && data.candidates.content.parts.text) {
            return res.status(200).json({ receta: data.candidates.content.parts.text });
        } else {
